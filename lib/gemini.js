...
━━━━━━ 営業クロージングテクニック ━━━━━━
以下5つのテクニックを理解・活用できているかを評価する。
1. HiGH&LOW（価格アンカリング）：高い選択肢・条件を先に提示してから本命の提案を出すことで、相手に「割安感・お得感」を感じさせる対比効果。
2. バンドワゴン（仲間意識）：「〇〇エリアの8割が導入済み」「同じ状況の方はほぼ全員決めている」など、周囲との同調心理を刺激して安心感でインパルスを上げる。
3. stress the deal（価値の強調）：金額・条件・メリットを具体的な数字や事実で積み上げ、「これだけの価値がある」と相手に感じさせて行動理由を作る。
4. fear of loss（限定意識）：「今だけ」「この機会を逃すと次はない」という希少性・緊急性でインパルスを上げ、決断を先延ばしするコストを意識させる。
5. インディファレンス（どっちでもいいよ感）：「もちろん決めなくても全然いいですよ」という余裕・無関心のスタンスを見せることで、相手の防衛心を下げ逆に「自分で決めたい」という能動性を引き出す。
...
3. 内容：精神論・道徳論ではなく、なぜ必要なのかロジックと仕組みで論理的に解説しているか。5ステップスとの関連・必要性が語られているとさらに加点。クロージングテクニック（HiGH&LOW/バンドワゴン/stress the deal/fear of loss/インディファレンス）への言及・理解があるとさらに加点。
4. 一般的な例：営業未経験者でも1秒で直感的に納得できる日常例か。
5. 稼働における例：明日の現場でそのままイメージして使えるほどリアルで生々しいシーンか。5ステップスのいずれかのシーン・またはクロージングテクニックの活用シーンに紐づいているとさらに加点。
...
必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  ...
  "closingTechniquesCheck": "内容・稼働例に5つのクロージングテクニック（HiGH&LOW/バンドワゴン/stress the deal/fear of loss/インディファレンス）のどれが使われていてどれが不足しているかの一言指摘",
