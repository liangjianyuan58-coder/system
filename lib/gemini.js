...
━━━━━━ 大前提：初めて聴く人の視点で評価する ━━━━━━
このアウトプットを「業界用語を知らない初めての聴き手」が聞いた場合、以下の3点が達成されているかが採点の根幹である。
① 内容を理解できるか（説明量は十分か・専門用語に説明はあるか・論理のつながりが見えるか）
② インパルス（心の熱量・行動意欲）が上がるか
③ 行動しようという気持ちになるか
どれほど形式が整っていても「理解→インパルス→行動」の流れが成立しないアウトプットには高得点を与えない。
...
4. 一般的な例：営業未経験者でも1秒で直感的に納得できる日常例か。かつ内容で説明した側面を具体化した例になっているか（内容と関係のない例はAP理解を妨げるため減点）。
   ▸ 0-3点：例が難解・業界用語だらけ・内容と無関係で混乱を招く
   ▸ 4-6点：例はあるが「確かに！」の瞬発力がない・内容との繋がりが弱い
   ▸ 7-8点：わかりやすい例で内容と繋がっているが、APへの橋渡しとしての強度が弱い
   ▸ 9-10点：「そういうことか！」と1秒で腑に落ち、内容→例→APの流れが自然につながる対比が明確な例

5. 稼働における例：明日の現場でそのままイメージして使えるほどリアルで生々しいシーンか。かつ内容で説明した側面を現場に落とした例になっているか（内容と無関係な現場話はNG）。アプローチ話法・インパルスファクターの活用シーンに紐づいているとさらに加点。
   ▸ 0-3点：抽象的・現場感ゼロ・または内容で説明した側面と全く無関係な現場話
   ▸ 4-6点：現場の話はあるが生々しさが弱い・内容との繋がりが弱い
   ▸ 7-8点：生々しい現場シーンで内容と繋がっているが、アプローチ話法やインパルスファクターとの紐づけが弱い
   ▸ 9-10点：内容→稼働例→APの流れが一本線で繋がり、アプローチ話法/インパルスファクターが実際に使われている生々しい架電・現場シーン
...
8. AP（最重要）：「意識します」「頑張ります」は一発不合格。誰が見ても実行したか判断できる行動ベース（分・秒単位・件数など数値や事実で確認できるレベル）で具体化されているか。さらに「内容で説明した側面」とAPが一致していること（例：内容で笑顔の話だけしたなら、APも笑顔に関する行動であるべき。内容と無関係なAPは大幅減点）。
   ▸ 0-2点：「意識します」「頑張ります」「心がけます」等の精神論AP（即失格）
...
必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "reasoning": {
    ...
    "consistency": "一貫性の分析：内容でどの側面・テーマを取り上げたかを特定し、一般例・稼働例がその説明を具体化しているか、APがその流れに沿っているかを確認。内容→例→APで一本線が通っているかを指摘（2〜3文）",
    "clarity": "初めて聴く人の視点での総評：業界用語・専門用語を説明なしで使っていないか／説明量は初心者が理解できる十分さか／全体の流れで理解→インパルス上昇→行動意欲につながるかを分析（2〜3文）"
  },
  ...
  "stepsCheck": "アウトプット全体の流れが営業5ステップスを意識した構成になっているかの評価：T-UPが導入として機能しているか／内容・例がショートストーリー+プレゼンの流れになっているか／APのT-UP→APがクロージングとして機能しているか。流れとして自然に5ステップスが体現されているかを指摘（1〜2文）",
  "consistencyCheck": "内容→例→APの一貫性評価：内容で取り上げた側面を例が具体化し、APがその流れに沿っているかを指摘。例やAPが内容と無関係なテーマになっていれば具体的に指摘（1〜2文）",
  "clarityCheck": "初めて聴く人が①理解できるか②インパルスが上がるか③行動したくなるかの3点評価：説明不足・専門用語の無説明使用・流れの断絶があれば具体的に指摘。問題なければその旨を伝える（1〜2文）",
