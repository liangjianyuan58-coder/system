...
export async function gradeOutput(steps) {
  const stepDefs = MODULES[ACTIVE_MODULE].steps;
...
特に次の2点を最重視すること：
- T-UP：冒頭で答え（「ハブファン」等のワード自体）をバラしていないか。
...
これは「こじつけ力強化モード」です。目的は、Have fun の本質（楽しむ＝スキル、リフレーミング、作業→ゲーム化、楽しい−悲しい=Have fun 等）を、
一見まったく無関係な単語に強引かつ論理的に結びつけて説明する「こじつけ力＝リフレーミング筋力」を鍛えることです。
ユーザーは与えられた単語を使ってハブファンを説明します。あなたはそれを評価し、さらに同じ単語を使ったお手本のこじつけ説明を示してください。

評価観点：
- ハブファンの本質に芯を食って結びついているか（ただ単語を出しただけ＝低評価）。
...
必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "score": 0,
  "good": "良かった点（1〜2文、熱く）",
  "improvement": "もっとこじつけ力・ハブファン度を上げる具体的な指摘（1〜2文）",
...
これは「リフレーミング・ジム」です。目的は第二部の核心「楽しいこと（主観的工夫）− 悲しいこと（客観的事実）＝ Have fun」を、
現場で実際に起きるネガティブな事象に対して鍛えることです。
...
export async function modelScript(theme) {
  const t = (theme || '').trim();
  const system = `${PERSONA}
...
完璧に踏んだ「Have fun の模範説明スクリプト」を生成してください。
ルール：
- T-UPでは答え（「ハブファン」というワード自体）をバラさない。
