...
4. 一般的な例：営業未経験者でも1秒で直感的に納得できる日常例か。かつ内容で説明した側面を具体化した例になっているか（内容と関係のない例はAP理解を妨げるため減点）。ある程度の誇張はOKだが、フィクションに寄りすぎて「自分には関係ない話」になるのはNG。聴き手が「自分ごと」として想像できる範囲に収まっているかを確認する。
   【比較の鉄則】対比・比較を使う場合、変えていい変数は「そのモジュールの概念を使っているか／いないか」の1点のみ。状況・環境・前提条件（ゲームのルール・職場・客層・商材など）を比較対象間でズラしてしまうと「条件が違うから差が出ただけ」になり例の証明力がゼロになる。前提条件がズレている比較例は大幅減点。
   ▸ 0-3点：例が難解・業界用語だらけ・内容と無関係で混乱を招く、または現実離れしすぎて誰も自分ごとにできない、または比較対象の前提条件が揃っておらず概念の効果が証明できていない
   ▸ 4-6点：例はあるが「確かに！」の瞬発力がない・内容との繋がりが弱い・または前提条件が微妙にズレていて比較の説得力が落ちている
   ▸ 7-8点：わかりやすい例で内容と繋がっており前提条件も揃っているが、APへの橋渡しとしての強度が弱い
   ▸ 9-10点：「そういうことか！」と1秒で腑に落ち、誰でも自分ごとにできる身近さがあり、比較対象の前提条件を完全に揃えた上で「概念あり vs なし」だけで結果の差を見せており、内容→例→APの流れが自然につながる対比が明確な例
...
5. 稼働における例：明日の現場でそのままイメージして使えるほどリアルで生々しいシーンか。かつ内容で説明した側面を現場に落とした例になっているか（内容と無関係な現場話はNG）。ある程度のドラマチックな演出はOKだが、フィクションに寄りすぎて「そんな状況ありえない」となるのはNG。聴き手が「明日の自分の稼働でありえる」と想像できる範囲に収まっているかを確認する。アプローチ話法・インパルスファクターの活用シーンに紐づいているとさらに加点。
   【比較の鉄則】稼働例でも対比・比較を使う場合、変えていい変数は「そのモジュールの概念を使っているか／いないか」の1点のみ。同じ架電状況・同じお客様の反応・同じ商材という条件で比べること。前提条件を変えると「状況が違うから結果が違っただけ」になり、稼働例としての証明力が失われる。
   ▸ 0-3点：抽象的・現場感ゼロ・内容と無関係・または非現実的すぎて誰も自分ごとにできない、または比較の前提条件がズレていて概念の効果が証明できていない
   ▸ 4-6点：現場の話はあるが生々しさが弱い・内容との繋がりが弱い・または前提条件のズレで比較の説得力が落ちている
   ▸ 7-8点：生々しい現場シーンで内容と繋がっており前提条件も揃っているが、アプローチ話法やインパルスファクターとの紐づけが弱い
   ▸ 9-10点：内容→稼働例→APの流れが一本線で繋がり、誰でも「明日の自分の現場」として想像できるリアルさがあり、前提条件を揃えた比較で概念の効果を証明した上で、アプローチ話法/インパルスファクターが実際に使われている生々しい架電・現場シーン
...
━━━━━━ stepNotes（ステップ別詳細注釈）━━━━━━
全ステップについて以下のルールでオブジェクトを書くこと。特に書くべきことがなければ null にする。

【8点以上のステップ → good オブジェクト】
- "good"：このステップが高得点だった理由を具体的に言語化する。どのテクニック・視点・構造が効いたかを明確にし、「次のアウトプットでも意識的に再現すべき強み」として書く（1〜2文）

【7点以下のステップ → bad オブジェクト】
- "issue"：問題の概要を一言で（例：「論理ギャップ」「比較の前提条件がズレている」）
- "whyBad"：聴き手が違和感を感じる理由・論理の破綻・因果関係のギャップを詳しく（2〜3文）。「〇〇という主張なのに〇〇という行動は矛盾する」「〇〇という結論を出すには〇〇の証明が必要だが抜けている」のように具体的に指摘すること。
- "howToFix"：「こう書き直す」という具体的な文章で示す（1〜2文。「数値化してください」などの抽象指示はNG）
...
━━━━━━ directFix（書き直し例）━━━━━━
"directFix" には「最もスコアが低いステップ1つ」を選び、そのステップをそのまま使えるレベルの具体的な例文に書き直すこと。抽象的な指示（「数値化してください」等）は絶対にNG。実際に口で読める完全文を書く。

━━━━━━ フィードバックの書き方（改善ポイント・コメント）━━━━━━
"improvements" 配列と "comment" フィールドは「答えを与える」形で書いてはならない。
必ず「問い」か「チャレンジ」の形で書くこと。

✗ NG例：「APに数値を入れてください」「比較の前提条件を揃えてください」
✓ OK例：「このAPを第三者が達成したか確認するとしたら、どう判断できますか？」
✓ OK例：「2つの例で唯一変わっている条件はAPの概念だけですか？他に変わっている前提条件はありますか？」

原則：
- 「〇〇してください」「〇〇が必要です」は禁止
- 相手が自分で気づくための問いを立てる
- 1つの問いで1つの気づきを促す（複数の問題を1文に詰め込まない）
- "stepNotes" の "howToFix" は具体的な例文なので問い形式でなくてOK
...
必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "reasoning": {
    ...
    "apUnderstanding": "AP理解度の分析：このAPを『なぜ』設定したのかの根拠がアウトプット全体を通じて説明されているか。モジュール（${moduleName}）の本質とAPの行動の間にある因果関係を発表者が理解した上で言語化しているか、それとも『なんとなくこれをやれ』で終わっているかを判定する。APの内容自体の適切さも評価すること：このモジュールの本質を体現するAPとして本当に最適か、より核心を突いたAPがあれば具体的に示す（2〜3文）",
  ...
  "apUnderstandingCheck": "AP理解度評価：このAPを設定した『理由・根拠』がアウトプット全体で説明されているかの評価。『${moduleName}だからこそこの行動』という因果関係が発表者の言葉で説明されているか、それとも『なんとなくやれ』になっているかを判定する。理解が浅い場合は『なぜこのAPなのかの説明がなく、モジュールとAPの繋がりが見えない』と指摘（2〜3文）",
  "apAppropriatenessCheck": "AP適切性評価：このモジュール（${moduleName}）の本質を体現するAPとして、設定された行動が本当に最適かを評価する。APがモジュールの核心を日常行動として正しく落とし込めているか、概念を表面的になぞっただけで本質からズレていないかを判定する。より適切なAPがある場合は『例えば〇〇のようなAPにすると、モジュールの本質をより体現できる』と具体的に代替案を提示すること（2〜3文）",
  ...
  "directFix": {
    "targetStep": "最もスコアが低いステップのキー（tup/conclusion/content/example/workExample/reconclusion/apTup/ap）",
    "targetLabel": "そのステップの日本語ラベル（T-UP / 結論 / 内容 / 一般的な例 / 稼働における例 / 再結論 / APのT-UP / AP）",
    "rewrite": "提出されたステップをこう書き直せ。そのまま口で読めるレベルの具体的な例文（接続詞・主語もすべて入れた完全文）。抽象的な指示ではなく実際の言葉を書くこと"
  },
  "languageCheck": {
    "score": 0,
...
あなたは「${moduleName}」の本質的な理解度を確認する4択問題を作る役割です。
表面的な知識ではなく、本質を掴んでいるかを問う問いを作ってください。

条件：
- 「${moduleName}とは何ですか？」のような単純な定義問いはNG
- 「なぜ〇〇なのか」「〇〇がない人はどうなるか」「〇〇と△△の違いは何か」など本質を問う
- 正解は1つ。残り3つはもっともらしいが本質を外した選択肢にする
- correctIndex はランダムに0〜3のどれかにすること（毎回Aが正解にならないようにする）
- explanation は正解の理由と、よくある誤解への反論を含める
...
必ず次のJSONのみを出力：
{
  ...
  "correctIndex": 0,
  "explanation": "正解の解説（2〜3文。なぜこれが正解でほかが違うのかを具体的に）"
}`;
  return runJSON(system, `「${moduleName}」の4択理解チェック問題を作成してください。`, 0.7);
}
...
// ── ⑧ 穴埋めテンプレート生成 ──
export async function modelTemplate(theme, moduleId) {
  const t = (theme || '').trim();
  const mid = moduleId || ACTIVE_MODULE;
  const moduleName = MODULES[mid].manual.title;
  const system = `${getPersona(mid)}
...
これは「穴埋めテンプレート生成」です。
「${moduleName}」の8ステップアウトプットの「型（テンプレート）」を生成してください。

【重要：型の書き方ルール】
- 文の構造・つなぎ言葉・論理の流れは具体的に書く
- エピソード・体験談・具体例・数字などの「中身」の部分は【　】で示す
- 各【　】には括弧内に何を入れるかを短く書く（例：【自分の失敗体験を1文で】）
- 使う人が【　】の部分だけ自分の言葉に置き換えればそのまま使えるレベルにする
- 【　】は1ステップに2〜4個が目安。全部埋めれば合格点が取れる構造にする

各ステップの型の条件：

【T-UP】greed factorで興味を引く構造。「${moduleName}」というワードは出さない。
例の型：「みなさん、【自分の失敗・困った経験を1文で】って体験ありますよね？実はこれ、【それが起きる根本原因を1文で】が原因なんですよ。聞いてくれたら今日の稼働が確実に変わるので、少しだけ時間ください。」

【結論】「${moduleName}」を初めて明かし核心を言い切る。

【内容】ロジックで説明する。精神論NG。

【一般的な例】前提条件を揃えた上で「概念あり vs なし」の対比を見せる。

【稼働における例】現場の架電シーンを具体的に描く。

【再結論】最初より確信が深まる言い切り。

【APのT-UP】APの内容をバラさずインパルスを引き上げる。

【AP】いつ・何を・何回/何分の数値まで型に入れる。

必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "tup": "【　】を使った型テキスト",
  "conclusion": "【　】を使った型テキスト",
  "content": "【　】を使った型テキスト",
  "example": "【　】を使った型テキスト",
  "workExample": "【　】を使った型テキスト",
  "reconclusion": "【　】を使った型テキスト",
  "apTup": "【　】を使った型テキスト",
  "ap": "【　】を使った型テキスト"
}`;

  const user = t
    ? `次のテーマ/状況に寄せた型を作ってください：「${t}」`
    : `標準的な型を1つ作ってください。`;
  return runJSON(system, user, 0.7);
}
...
export async function rewriteStep({ moduleId, stepKey, stepLabel, original, issue, howToFix }) {
  const mid = moduleId || ACTIVE_MODULE;
  const moduleName = MODULES[mid].manual.title;
  const system = `${getPersona(mid)}
...
あなたは8ステップアウトプットのコーチです。提出された1ステップの文章を採点基準の満点条件を満たすように書き直してください。

ルール：
- そのまま口で読める完全な文章で書く（接続詞・主語もすべて含む）
- 抽象的な指示ではなく実際の言葉を書く
- 元の文に良い部分があれば活かす
- 対象ステップ：${stepLabel}（モジュール：${moduleName}）

必ず次のJSONのみを出力：
{
  "rewritten": "書き直した完全文"
}`;
  const user = `【ステップ】${stepLabel}\n【元の文】\n${original || '(空欄)'}\n\n【問題点】${issue || ''}\n【修正の方向性】${howToFix || ''}`;
