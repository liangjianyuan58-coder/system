...
// ── ステップ別ヒント生成（行き詰まり時のソクラテス的問い）──
export async function generateStepHint({ moduleId, stepKey, stepLabel, issue, whyBad }) {
...
あなたは「${moduleName}」のアウトプット採点コーチです。
受講者が「${stepLabel}」ステップで2回書き直しても改善できていません。

問題の核心：${issue}
なぜ問題か：${whyBad}

受講者が自力で気づけるよう、方向を示す「問い」を1つだけ返してください。

ルール：
- 答えや修正例を絶対に言わない
- 「〇〇してください」「〇〇すればいい」は禁止
- 「〇〇という観点で見ると、どうなりますか？」のような問いかけの形にする
- 1文のみ。短く鋭く。

必ず次のJSONのみを出力：
{ "hint": "問いの文（？で終わること）" }`;

  return runJSON(system, `「${stepLabel}」で行き詰まっています。問いを1つください。`, 0.7);
}
