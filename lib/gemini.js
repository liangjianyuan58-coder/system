...
4. 一般的な例：営業未経験者でも1秒で直感的に納得できる日常例か。
   ▸ 0-3点：例が難解・業界用語だらけ・関連性が不明確
   ▸ 4-6点：例はあるが「確かに！」の瞬発力がない・対比がない
   ▸ 7-8点：わかりやすい例だが意外性・インパクトが弱い
   ▸ 9-10点：「そういうことか！」と1秒で直感的に腑に落ちる身近な例で対比が明確

5. 稼働における例：明日の現場でそのままイメージして使えるほどリアルで生々しいシーンか。5ステップスのいずれかのシーン・またはアプローチ話法・インパルスファクターの活用シーンに紐づいているとさらに加点。
   ▸ 0-3点：抽象的・現場感ゼロ・「なんとなくやる」レベルの話
   ▸ 4-6点：現場の話はあるが生々しさ・リアルさが弱い
   ▸ 7-8点：生々しい現場シーンがあり実践イメージできるが、フレームワーク紐づけが弱い
   ▸ 9-10点：5ステップスの具体的シーンかつアプローチ話法/インパルスファクターが実際に使われている生々しい架電・現場シーン
...
必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "reasoning": {
    ...
    "consistency": "内容一貫性の分析：内容・稼働例でどの側面・テーマを取り上げたかを特定し、APがその説明と一致しているかを確認。内容で触れていないテーマのAPになっていないかを指摘（1〜2文）",
  ...
  "consistencyCheck": "内容で取り上げた側面・テーマとAPが一致しているかの評価：内容で説明した点に沿ったAPになっているか、内容で触れていないことのAPになっていないかを一言で指摘",
