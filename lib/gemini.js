...
function parseJSON(text) {
  ...
  try {
    ...
    const preview = t.length > 300 ? t.slice(0, 300) + '…(省略)' : t;
    throw new Error(
      `AIのJSON解析に失敗しました。\n` +
      `原因: ${err.message}\n` +
      `受け取った応答 (${t.length}文字):\n${preview}`
...
function resolveModule(moduleId) {
  ...
  if (!MODULES[mid]) throw new Error(`モジュール "${mid}" が見つかりません。`);
  return mid;
}
...
export async function gradeOutput(steps) {
  const mid = resolveModule(steps.moduleId);
...
// ── ② こじつけ力強化モードの査定＋お手本例 ──
export async function kojitsukeFeedback(word, output, moduleId) {
  const mid = resolveModule(moduleId);
  const moduleName = MODULES[mid].manual.title;
  const system = `${getPersona(mid)}
...
export async function reframeFeedback(situation, userReframe, moduleId) {
  const mid = resolveModule(moduleId);
  const system = `${getPersona(mid)}
...
// ── ④ 北極星 AI振り返り ──
export async function northStarReview(northStar, recentOutputs) {
  const apList = recentOutputs.length > 0
    ? recentOutputs.map((ap, i) => `${i + 1}. ${ap}`).join('\n')
    : '（先週のAPなし）';

  const system = `あなたは厳しくも温かい、プロのビジネスコーチです。
ユーザーが設定した「北極星（長期目標）」と、先週実際に書いたAP（アクションプラン）を比較し、
行動が目標に向かっているかを率直かつ熱く評価してください。

必ず次のJSONのみを出力（前後に説明文やコードフェンスを付けない）：
{
  "alignment": 0,
  "comment": "総合評価コメント（2〜3文、率直に）",
  "good": "北極星に向かっていた行動の良かった点（1〜2文）",
  "challenge": "北極星に近づくための課題（1〜2文）",
  "nextAP": "来週の具体的な行動提案（1〜2文、行動ベース）"
}
（alignment は0〜10の整数。10=完全整合、0=全く無関係）`;
