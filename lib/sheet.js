...
async function getSheet() {
  ...
  if (missingHeaders.length > 0) {
    try {
      ...
      // setHeaderRow は columnCount を超えるとエラーになるので先にリサイズ
      if (newHeaders.length > (sheet.columnCount || 26)) {
        await sheet.resize({ columnCount: newHeaders.length + 5 });
      }
...
// ── IDで採点結果を更新（結果ページでAI採点後に書き戻す）──
export async function updateOutputById(id, data) {
  if (!id) return;
  const sheet = await getSheet();
  const rows = await sheet.getRows();
  const row = rows.find((r) => (r.get(C.id) || '') === id);
  if (!row) return;
  if (data.total != null) row.set(C.total, String(data.total));
  if (data.verdict != null) row.set(C.verdict, data.verdict);
  if (data.good != null) row.set(C.good, data.good);
  if (data.improvements != null) row.set(C.improvements, data.improvements);
  if (data.comment != null) row.set(C.comment, data.comment);
  if (data.rawJson != null) row.set(C.rawJson, data.rawJson);
  if (data.consistencyCheck != null) row.set(C.consistencyCheck, data.consistencyCheck);
  if (data.languageScore != null) row.set(C.languageScore, data.languageScore);
  await row.save();
}
