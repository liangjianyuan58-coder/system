...
// 元の列順を維持し、新列を末尾に追加（既存シートへの後方互換）
const HEADER = [
  'タイムスタンプ','ユーザーID','名前',
  '①T-UP','②結論','③内容','④一般的な例','⑤稼働における例','⑥再結論','⑦AP（アクションプラン）',
  'モジュール','お手本使用','AI合計点','AI判定',
];
const NEW_COLS = ['モジュール','お手本使用','AI合計点','AI判定'];
...
async function getSheet() {
  ...
  if (hv.length === 0 || !hv.includes(C.uid)) {
    ...
    // 既存シートに新カラムが欠けていれば末尾に追加
    const missing = NEW_COLS.filter((col) => !sheet.headerValues.includes(col));
    if (missing.length > 0) {
...
function rowToObj(r) {
  const aiTotalRaw = r.get(C.aiTotal);
  return {
    type: 'output',
    ts: r.get(C.ts) || '',
    userId: r.get(C.uid) || '',
    name: r.get(C.name) || '(名無し)',
    moduleId: r.get(C.module) || '',
    usedModel: r.get(C.usedModel) === '1',
    aiTotal: aiTotalRaw !== '' && aiTotalRaw != null ? Number(aiTotalRaw) : null,
    aiVerdict: r.get(C.aiVerdict) || '',
    tup: r.get(C.tup) || '',
...
export async function appendOutput(data) {
  ...
  await sheet.addRow({
    ...
    [C.ap]: data.ap,
    [C.module]: data.moduleId || '',
    [C.usedModel]: data.usedModel ? '1' : '',
  });
...
// ── AI採点をメインシート行に後書き ──
export async function saveGrade(userId, ts, total, verdict) {
  const sheet = await getSheet();
  const rows = await sheet.getRows();
  const row = rows.find((r) => r.get(C.uid) === userId && r.get(C.ts) === ts);
  if (!row) return;
  row.set(C.aiTotal, String(total));
  row.set(C.aiVerdict, verdict || '');
  await row.save();
}
...
const TL_HEADER = Object.values(TL);

async function getTrainingLogSheet() {
  const doc = new GoogleSpreadsheet(process.env.SHEET_ID, getAuth());
  await doc.loadInfo();
  let sheet = doc.sheetsByTitle['トレーニングログ'];
  if (!sheet) {
    sheet = await doc.addSheet({ title: 'トレーニングログ', headerValues: TL_HEADER });
  } else {
    let loaded = false;
    try { await sheet.loadHeaderRow(); loaded = true; } catch (_) {}
    const hv = loaded ? (sheet.headerValues || []) : [];
    if (hv.length === 0 || !hv.includes(TL.uid)) {
      await sheet.setHeaderRow(TL_HEADER);
      await sheet.loadHeaderRow();
    }
  }
  return sheet;
...
export async function appendTrainingLog({ userId, name, type, moduleId, input1, input2, aiScore }) {
  const sheet = await getTrainingLogSheet();
  await sheet.addRow({
    [TL.ts]: new Date().toISOString(),
    [TL.uid]: userId || '',
    [TL.name]: name || '(名無し)',
    [TL.type]: type || '',
    [TL.module]: moduleId || '',
    [TL.input1]: input1 || '',
    [TL.input2]: input2 || '',
    [TL.aiScore]: aiScore != null ? String(aiScore) : '',
  });
}

export async function listTrainingLogs({ scope = 'all', userId = '', limit = 50 } = {}) {
  const sheet = await getTrainingLogSheet();
  const rows = await sheet.getRows();
  let list = rows.map((r) => {
    const scoreRaw = r.get(TL.aiScore);
    return {
      type: r.get(TL.type) || 'kojitsuke',
      ts: r.get(TL.ts) || '',
      userId: r.get(TL.uid) || '',
      name: r.get(TL.name) || '(名無し)',
      moduleId: r.get(TL.module) || '',
      input1: r.get(TL.input1) || '',
      input2: r.get(TL.input2) || '',
      aiScore: scoreRaw !== '' && scoreRaw != null ? Number(scoreRaw) : null,
    };
  });
  if (scope === 'me') list = list.filter((o) => o.userId === userId);
  list.sort((a, b) => new Date(b.ts) - new Date(a.ts));
...
const SS_HEADER = Object.values(SS);
const SESSION_TTL_MS = 10 * 60 * 1000; // 10分

async function getSessionSheet() {
  const doc = new GoogleSpreadsheet(process.env.SHEET_ID, getAuth());
  await doc.loadInfo();
  let sheet = doc.sheetsByTitle['セッション'];
  if (!sheet) {
    sheet = await doc.addSheet({ title: 'セッション', headerValues: SS_HEADER });
  } else {
    let loaded = false;
    try { await sheet.loadHeaderRow(); loaded = true; } catch (_) {}
    const hv = loaded ? (sheet.headerValues || []) : [];
    if (hv.length === 0 || !hv.includes(SS.uid)) {
      await sheet.setHeaderRow(SS_HEADER);
      await sheet.loadHeaderRow();
    }
  }
  return sheet;
...
export async function getLineSession(userId) {
  if (!userId) return null;
  const sheet = await getSessionSheet();
  const rows = await sheet.getRows();
  const row = rows.find((r) => (r.get(SS.uid) || '') === userId);
  if (!row) return null;
  const createdAt = Number(row.get(SS.createdAt) || 0);
  if (Date.now() - createdAt > SESSION_TTL_MS) {
    await row.delete();
    return null;
  }
  const extraRaw = row.get(SS.extra) || '';
  let extra = {};
  try { extra = extraRaw ? JSON.parse(extraRaw) : {}; } catch {}
  return { action: row.get(SS.action) || '', moduleId: row.get(SS.moduleId) || '', extra };
}

export async function setLineSession(userId, data) {
  if (!userId) return;
  const sheet = await getSessionSheet();
  const rows = await sheet.getRows();
  const row = rows.find((r) => (r.get(SS.uid) || '') === userId);
  const extraStr = data.extra ? JSON.stringify(data.extra) : '';
  if (row) {
    row.set(SS.action, data.action || '');
    row.set(SS.moduleId, data.moduleId || '');
    row.set(SS.extra, extraStr);
    row.set(SS.createdAt, String(Date.now()));
    await row.save();
  } else {
    await sheet.addRow({
      [SS.uid]: userId,
      [SS.action]: data.action || '',
      [SS.moduleId]: data.moduleId || '',
      [SS.extra]: extraStr,
      [SS.createdAt]: String(Date.now()),
    });
  }
}

export async function clearLineSession(userId) {
  if (!userId) return;
  const sheet = await getSessionSheet();
  const rows = await sheet.getRows();
  const row = rows.find((r) => (r.get(SS.uid) || '') === userId);
